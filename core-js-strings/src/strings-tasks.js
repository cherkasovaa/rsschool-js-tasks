...
function formatTime(minutes, seconds) {
  const formatter = (n) => String(n).padStart(2, '0');
...
function countVowels(str) {
  return str.split('').reduce((sum, item) => {
    let counter = sum;
    if ('aeiouyAEIOUY'.includes(item)) counter += 1;
    return counter;
  }, 0);
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (lowerArray.indexOf(char) >= 0) {
      char = encodeChar(lowerArray, char);
    } else if (upperArray.indexOf(char) >= 0) {
      char = encodeChar(upperArray, char);
    }
