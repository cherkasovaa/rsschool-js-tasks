...
function getNextFriday(date) {
  ...
  const fridayIdx = 5;
  let dayUntilFriday = (fridayIdx - currentDay + 7) % 7;

  if (dayUntilFriday === 0) dayUntilFriday = 7;

  newDate.setDate(newDate.getDate() + dayUntilFriday);
...
function isDateInPeriod(date, period) {
  ...
  const endPeriod = new Date(end).getTime();

  return currentDate >= startDate && currentDate <= endPeriod;
}
